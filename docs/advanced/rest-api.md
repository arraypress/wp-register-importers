...
The library registers 7 REST API endpoints under the `importers/v1` namespace. All require the configured capability (default: `manage_options`) and a valid WordPress REST nonce via the `X-WP-Nonce` header.
...
Validates all rows through the field pipeline without calling `process_callback`. Reports valid count, error count, and detailed error messages with row numbers.
...
```json
{
    ...
    "error_count": 2,
    "errors": [
        { "row": 15, "item": "bad-sku", "message": "Price is required." },
        { "row": 42, "item": "test@bad", "message": "Email must be a valid email address." }
    ]
}
```
...
**Response includes:** processed count, created/updated/skipped/failed counts, errors, `has_more` flag, next offset, percentage, cumulative stats.
